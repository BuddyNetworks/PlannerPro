...
public static class PlannerEndpoints
{
    ...
    public static IEndpointRouteBuilder MapPlannerApi(this IEndpointRouteBuilder app)
    {
        ...
        api.MapGet("/sprints/{id:int}/board", async (int id, PlannerDbContext db) =>
        {
            ...
            foreach (var p in projects)
            {
                ...
                var tasks = goal?.Tasks.OrderBy(t => t.SortOrder).ThenBy(t => t.Id)
                    .Select(t => new TaskDto(t.Id, t.Label, t.IsDone, (int)t.Points, t.Priority, t.SortOrder))
                    .ToList() ?? [];
                ...
                columns.Add(new BoardColumnDto(
                    ...
                    goal?.Id, goal?.GoalText, goal?.Status, points, tasks));
            }
            ...
            return Results.Ok(new BoardDto(
                ...
                prev, next,
                total, OverloadThreshold, total > OverloadThreshold,
                columns));
        });
            ...
            async (int sprintId, int projectId, UpsertGoalRequest req, PlannerDbContext db) =>
        {
            ...
            if (goal is null)
            {
                ...
                goal = new SprintGoal { SprintId = sprintId, ProjectId = projectId, GoalText = req.GoalText.Trim(), Status = req.Status };
            ...
            return Results.Ok(new { goal.Id, goal.GoalText, goal.Status });
        });
            ...
            async (int sprintId, int projectId, CreateTaskRequest req, PlannerDbContext db) =>
        {
            ...
            return Results.Ok(new TaskDto(task.Id, task.Label, task.IsDone, (int)task.Points, task.Priority, task.SortOrder));
        });
    ...
    private static async ValueTask<object?> AntiforgeryFilter(
